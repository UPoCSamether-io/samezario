...
const BASE_TURN = 3.3;    // rad/s（小型時の上限）
...
export function startGame({ canvas, mini, sharkId, map, onEnd, onHud, attract = false, net = null, name = 'YOU' }) {
  ...
  function step(dt) {
    ...
    // 1280/base に落ち着き、画面幅によらず一定になる（実測: 844px も 1280px も 1219）。
    // 上限を 1 で止めてあるので 1280 より広い画面は補正されず、そのぶん広く見える
    // （実測: 1920px で 1829）。これはデスクトップの見え方を変えないための意図的な
    // 割り切りで、補正前の 2.27 倍差が 1.5 倍差に縮む、という改善に留まる。
