...
// v が付いていない保存（このスキーマ版導入より前の main.js 直書き時代のセーブ）は
// 互換として素通しし、defaults とマージして v を補う。v が付いていて値が食い違う
// ときだけ「既知だが読めない版」として捨てる。
function read() {
  try {
    const v = JSON.parse(localStorage.getItem(KEY) || '{}');
    if (!v || typeof v !== 'object') return {};
    if (v.v !== undefined && v.v !== DEFAULTS.v) return {};
    return v;
  } catch {
