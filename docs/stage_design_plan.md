...
| `tamagawa` | **多摩川** | 南部（新設） | 写真照合<br>（調布市郷土博物館） | **3帯カレント**: 上=河川敷 0px/s、中=流れ 48px/s、下=急流 110px/s。境界は±0.04vで連続的に混ぜる | 古代の麻布 |
...
- **ギミック**: **急流カレント（River Current）**
  - 外接矩形の上から、河川敷（0.00–0.34: 0px/s）・流れ（0.34–0.67: 48px/s）・急流（0.67–1.00: 110px/s）。向きは共通で、境界は±0.04vを線形に混ぜる。
  - 3帯とも水域があり、上は安全だが加速なし、下は乗れば約282px/s・逆らうと約62px/sになる選択を作る（2秒で 553px 対 114px の実測）。
...
| エリア | kind | 効き | 実測・根拠 |
| --- | --- | --- | --- |
| `tamagawa` | `current` | 速度ベクトルに `dir=0.32rad` を加算。強さは外接矩形の上下で 0 / 48 / 110px/s（±0.04v で線形に混ぜる） | 向きは輪郭の主軸の実測（18.3°）。2秒で進む距離は上から 334/333・429/237・553/114px（下流/上流）。上帯は加算なし＝河川敷で、流れに得も損もない |
