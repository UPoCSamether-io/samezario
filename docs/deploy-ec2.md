...
### クレジット満タン側の実測（2026-08-17 / 残 239）

同じ箱に、人数を段階的に増やしながら当てた（各40秒、下位25%が3秒に1回強制切断→即再接続。
`--churn-ms` を付けると入退室が混ざる）:

```bash
node scripts/loadtest.mjs --clients 40 --seconds 40 --churn-ms 3000 \
  --url wss://54-168-149-87.nip.io/ws
```

| 人数 | 部屋 | 受信レート 最悪 | 中央 | 配信間隔 p99 | 帯域 | 判定 |
| --- | --- | --- | --- | --- | --- | --- |
| 8 | 1 | 14.97 /s | 15.07 | 80ms | 0.14 GB/時 | 合格 |
| 16 | 2 | 15.00 | 15.05 | 78ms | 0.20 | 合格 |
| 24 | 2 | 15.00 | 15.05 | 76ms | 0.38 | 合格 |
| 32 | 3 | 14.95 | 15.00 | 88ms | 0.35 | 合格 |
| 40 | 3 | 15.05 | 15.10 | 83ms | 0.60 | 合格 |
| 56 | 5 | 14.90 | 15.07 | 99ms | 0.84 | 合格 |
| 72 | 6 | 14.10 | 14.82 | 123ms | 0.92 | 合格 |

**72人・6部屋でも盤面は痩せない。** サーバ CPU は idle 75〜85% のままで、
クレジットは試験中むしろ増えた（239.06 → 240.22）＝ 72人でも定常 0.2 vCPU の枠に収まる。
帯域は 0.92 GB/時 なので、無料枠 100GB は 72人フル稼働で約108時間ぶん。

72人の「最悪 14.10」は箱ではなく**計測側**。24人×3プロセスに分けると 14.65〜14.90 に戻る
（1プロセスで72接続ぶんの JSON を捌くと node が先に詰まる）。50人を超える計測は分割すること。

### 落とし穴: `nip.io` の名前解決が計測を汚す

高負荷中に入室（close → hello）が p95 2秒に見えることがあるが、**箱ではない**。切り分け:

| 測ったもの | 無負荷 | 72人＋入退室中 |
| --- | --- | --- |
| `/health`（同じ node を通る） | 74ms | 121ms（最大 330ms） |
| `nip.io` の DNS lookup | p50 30ms / p95 237ms | **p50 300ms / p95 921ms** |
| 入室 p95（再現試行 3回） | 170ms | 335 / 350 / 342ms |

サーバ側に2秒ぶんの同期処理は存在しない（`makeRoom` 2〜12ms、`step` p99 4.2ms、
`snapshot` 0.15ms）。node は DNS をキャッシュしないので、再接続のたびに `nip.io` を
引き直す負荷試験だけがこれを踏む。ブラウザはキャッシュするので実プレイヤーは1回だけ。

ただし **`nip.io` は無保証の第三者DNS で、無負荷でも p95 237ms・最大 320ms 出ている。**
ここが落ちると誰も接続できない。本番当日を控えるなら独自ドメイン（A レコードを EIP へ）に
移すのが、この構成に残っている唯一の単一障害点つぶし。
