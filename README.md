...
線の生死は ping ではなく**無言**で見る。全員が 15Hz 以上で送り続けているので、30秒黙った線は
落ちたか裏に回ったかのどちらかとして切る（`WS_DEAD_MS` で変更可）。これが無いと、FIN の来ない
落ち方をしたホストが `readyState 1` のまま残って部屋ごと凍る。タブを長く裏に回すと切れるのは仕様。
